...
def _get_bootstrap_args() -> tuple[str, Optional[list[str]], dict[str, str]]:
    ...
    # before monarch._rust_bindings (see monarch/__init__.py) so torch's HIP
    # runtime stays ahead of monarch's system libamdhip64 on ROCm, avoiding a
    # fatal rocprofiler-register abort. Gated on the parent actually using torch
    # so CPU / no-torch workloads never import it.
